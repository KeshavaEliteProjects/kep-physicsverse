...
export default function TopicView() {
  ...
  if (topic === false)
    return (
      <div className="p-10 text-muted-foreground">
        Topic not found.{" "}
        <Link to="/app/universe" className="text-primary">
          Back
        </Link>
      </div>
    );
  ...
  return (
    <div className="p-6 lg:p-10 max-w-5xl mx-auto">
      <button
        data-testid="topic-back-button"
        onClick={() => navigate("/app/universe")}
        className="flex items-center gap-1.5 text-sm text-muted-foreground hover:text-slate-900 mb-5"
      >
      ...
      <div className="flex items-start gap-4">
        ...
        <div>
          <h1 className="font-display font-bold text-3xl lg:text-4xl text-slate-900">
            {topic.name}
          </h1>
          ...
          <div className="flex items-center gap-4 mt-3 text-xs font-mono-data">
            <span className="text-muted-foreground">
              {topic.concept_count} concepts · {topic.sections.length} sections
            </span>
            {topic.interactive_count > 0 && (
              <span className="inline-flex items-center gap-1 text-emerald-600">
                <Play className="w-3 h-3 fill-emerald-600" />{" "}
                {topic.interactive_count} interactive
              </span>
            )}
          </div>
        </div>
        ...
        {topic.sections.map((sec, si) => (
          <div key={si} data-testid={`section-${si}`}>
            <div className="flex items-center gap-2 mb-3">
              <div
                className="w-6 h-6 rounded-md flex items-center justify-center text-xs font-mono-data font-semibold"
                style={{
              ...
              {sec.concepts.map((c, ci) =>
                c.sim ? (
                  <Link
                    ...
                    to={`/app/sim/${c.sim}`}
                    state={{
                      from: `/app/topic/${topicId}`,
                      fromLabel: topic.name,
                    }}
                    data-testid={`concept-sim-${c.sim}`}
                  ...
                  <Link
                    key={ci}
                    to={`/app/tutor?q=${encodeURIComponent("Explain " + c.name + " in " + topic.name + " with key equations.")}`}
      ...
      <div className="rounded-2xl border border-border bg-slate-50 p-6 mt-12 flex items-center gap-4">
        ...
        <div className="flex-1">
          <p className="font-display font-semibold text-slate-900">
            Learn any concept instantly
          </p>
          <p className="text-sm text-muted-foreground">
            Tap a concept to open the AI tutor, or a green one to run its live
            simulation.
          </p>
        </div>
      </div>
